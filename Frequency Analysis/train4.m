[gtr,FsG] = audioread('g4.wav');
[sax,FsS] = audioread('s4.wav');
...
[peaksG, locG] = findpeaks(P1G,fG, 'MinPeakDistance', 120, 'MinPeakHeight', 1e-4);
peaksG(1) = [];
locG(1) = [];
subplot(3,2,2);
scatter(locG, peaksG);
...
[peaksS, locS] = findpeaks(P1S,fS, 'MinPeakDistance', 120, 'MinPeakHeight', 1e-4);
subplot(3,2,4);
scatter(locS, peaksS);
xlim([0,6000]);
title('Sax Peaks');
