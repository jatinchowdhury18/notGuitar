[gtr,FsG] = audioread('g1.wav');
[sax,FsS] = audioread('s1.wav');
...
[peaksG, locG] = findpeaks(P1G,fG, 'MinPeakDistance', 110, 'MinPeakHeight', 3.5e-4);
peaksG(1) = [];
locG(1) = [];
subplot(3,2,2);
scatter(locG, peaksG);
...
[peaksS, locS] = findpeaks(P1S,fS, 'MinPeakDistance', 110, 'MinPeakHeight', 5e-5);
subplot(3,2,4);
scatter(locS, peaksS);
xlim([0,6000]);
title('Sax Peaks');
