[gtr,FsG] = audioread('g25.wav');
[sax,FsS] = audioread('s25.wav');
...
[peaksG, locG] = findpeaks(P1G,fG, 'MinPeakDistance', 450, 'MinPeakHeight', 1e-4);
peaksG(1) = [];
locG(1) = [];
subplot(3,2,2);
scatter(locG, peaksG);
...
[peaksS, locS] = findpeaks(P1S,fS, 'MinPeakDistance', 450, 'MinPeakHeight', 2e-5);
peaksS(1) = [];
locS(1) = [];
subplot(3,2,4);
scatter(locS, peaksS);
