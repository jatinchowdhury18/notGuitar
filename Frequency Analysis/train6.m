[gtr,FsG] = audioread('g6.wav');
[sax,FsS] = audioread('s6.wav');
...
[peaksG, locG] = findpeaks(P1G,fG, 'MinPeakDistance', 140, 'MinPeakHeight', 1e-4);
peaksG(1) = [];
locG(1) = [];
subplot(3,2,2);
scatter(locG, peaksG);
...
[peaksS, locS] = findpeaks(P1S,fS, 'MinPeakDistance', 140, 'MinPeakHeight', 1e-4);
%peaksS(1) = [];
%locS(1) = [];
subplot(3,2,4);
scatter(locS, peaksS);
